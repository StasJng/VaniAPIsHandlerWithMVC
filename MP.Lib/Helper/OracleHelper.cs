...
namespace MP.Lib.Helper
{
    ...
    public sealed class OracleHelper
    {
        ...
        private static void AssignParameterValues(OracleParameter[] commandParameters, object[] parameterValues)
        {
            ...
            foreach (OracleParameter p in commandParameters)
            {
                if (p.Direction == ParameterDirection.Input)
                {
                    ...
                    if (p.OracleDbType == OracleDbType.NVarchar2 || p.OracleDbType == OracleDbType.NChar || p.OracleDbType == OracleDbType.Blob)
                    {
                ...
                else if ((p.Direction == ParameterDirection.Output || p.Direction == ParameterDirection.InputOutput) && p.OracleDbType != OracleDbType.RefCursor)
                {
                ...
                else if (p.OracleDbType == OracleDbType.RefCursor)
                {
                    p.Value = null;
        ...
        private static DataSet ExecuteDataset(OracleConnection connection, CommandType commandType, string commandText, ref object[] outParam, params OracleParameter[] commandParameters)
        {
            ...
            foreach (OracleParameter p in commandParameters)
            {
                if (p.OracleDbType != OracleDbType.RefCursor && (p.Direction == ParameterDirection.Output || p.Direction == ParameterDirection.InputOutput))
                {
        ...
        public static object ExecuteScalar(IDbTransaction transaction, string spName, params object[] parameterValues)
        {
            ...
            else
            {
                ...
                {
                    AssignParameterValues(commandParameters, null);
                    return ExecuteScalar((OracleTransaction)transaction, CommandType.StoredProcedure, spName, commandParameters);
                }
                else
                {
    ...
    public sealed class OracleHelperParameterCache
    {
        ...
        public static OracleParameter[] DiscoverSpParameterSet(string connectionString, string spName)
        {
            using (OracleConnection cn = new OracleConnection(connectionString))
            using (OracleCommand cmd = new OracleCommand(spName, cn))
            {
                ...
                //        cmd.Parameters.RemoveAt(0);
                //}
                if (ParameterDirection.ReturnValue == cmd.Parameters[0].Direction || ParameterDirection.Output == cmd.Parameters[0].Direction)
                    cmd.Parameters.RemoveAt(0);
